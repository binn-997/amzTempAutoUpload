"""th（Top Herren 男士上装）品牌处理器 — 兼容入口。

用法：
    python prov_uploadEcel_Split_th.py

此文件为向后兼容入口。内部从 config/categories.yaml 读取 th 的默认配置，
委托通用 BaseProcessor 执行完整流水线。

如需指定非默认源文件，请使用 CLI 入口：
    python src/main.py process --type th --source 其他文件.xlsx
"""
