"""std（Strumpfhose Damen 女士连裤袜）品牌处理器 — 兼容入口。

用法：
    python prov_uploadEcel_Split_std.py

此文件为向后兼容入口。内部从 config/categories.yaml 读取 std 的默认配置，
委托通用 BaseProcessor 执行完整流水线。

如需指定非默认源文件，请使用 CLI 入口：
    python src/main.py process --type std --source 其他文件.xlsx
"""
